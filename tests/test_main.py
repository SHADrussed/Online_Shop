...
def test_add():
    assert (
        ...
        + Product(None, "1024GB, Синий", 31000.0, 14)
        == 900000 + 31000 * 14
    )
...
def test_category1(category_tv):
    ...
    assert (
        category_tv.products == """Наушники Picun, Цена: 1999 руб. Остаток: 12 шт.\n"""
    )
...
def test_category_multiple_products():
    ...
    assert (
        category.products
        == """Product1, Цена: 100.0 руб. Остаток: 10 шт.
...
def test_multiple_categories():
    ...
    category2 = Category(
        "Cat2", "Desc2", [Product("P2", "D2", 20.0, 2), Product("P3", "D3", 30.0, 3)]
    )
...
def test_add_product():
    ...
    category = Category(
        ...
        "Много спать пора",
        [
            Product("Собянин", "Москву построил", 777, 1),
...
def test_dictionary_convert():
    ...
    product = {
        "name": "Альбом Lonerism",
        "description": "Творчество Tame Impala",
        "price": 18000,
        "quantity": 32,
    }
...
def test_dictionary_convert_if_stance():
    ...
    product = {
        "name": "Les Paul",
        "description": "Офигенная гитара",
        "price": 48000,
        "quantity": 3,
    }
    actual = Product.new_product(
        product,
        [
            Product("Овощ", "Вкусный", 808080800, 213),
...
def test_smartphone_and_lawn_grass(smartphone_product):
    ...
    lawn_product = LawnGrass(
        "Трава",
        "Газончик, мой любимый. Ее обычно нехватает дотерам и тем, кто играет свип на гитаре",
        119000,
        1,
        "Кризкистан",
        "1 год",
    ...
    assert (
        str(smartphone_product) == "Redmi note 8 pro, Цена: 18000.0 руб. Остаток: 4 шт."
    )
